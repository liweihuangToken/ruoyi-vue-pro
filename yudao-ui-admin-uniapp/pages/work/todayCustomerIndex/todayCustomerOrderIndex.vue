	...
	<view class="container" style="background-color: #fff;">
		...
		<uni-list :border="true" v-for="(item, index) in swipeList" :key="index">
			<uni-card :cover="item.orderGoodsPictureUrl" @click="onClick" >
				...
				<view>
					<uni-table>
						<uni-tr>
							<uni-td style="font-weight: bold;">尺码:</uni-td>
							<uni-td style="font-weight: bold; color: red; font-size: 20px;">{{ item.size }}</uni-td>
							<uni-td style="font-weight: bold;">备注:</uni-td>
		...
		<!-- <view class="uni-padding-wrap uni-common-mt">
			<view class="button-sp-area">
				<button v-show="swipeList.length > 0 && selectedTags2[0] == 1" 
					type="warn" 
					plain="true" 
					class="warn-button-body"
					@click="oneClickToBalance()">一键结算
				</button>
			</view>
		</view> -->
		<view>
...
<script>
	...
	import { getTodayCustomerOrderList,
	...
	export default {
		...
		onLoad(options) {
		  // this.selectedTags[0] = "1";
		  this.downstreamAlias = options.downstreamAlias;
		  this.getTodayCustomerOrderList(options);
		},
		onPullDownRefresh() {
		    // 处理下拉刷新的逻辑
			// this.selectedTags2 = [];
		...
		methods: {
			...
			actionsClick(item){
				this.orderItem = item;
				this.orderType = 1;
				this.messageText = `请您确认是否对 ${item.orderCode} 订单进行入库操作？`
				this.$refs.popup.open('center')
			},
			actionsClickToNotStock(item){
				this.orderItem = item;
				this.orderType = 2;
				this.messageText = `请您确认是否对 ${item.orderCode} 订单进行未到货操作？`
				this.$refs.popup.open('center')
			},
			actionsClickToRegister(item){
				this.orderItem = item;
				this.orderType = 3;
				this.messageText = `请您确认是否对 ${item.orderCode} 订单进行重置为登记操作？`
				this.$refs.popup.open('center')
			},
			dialogClose() {
			...
			getTodayCustomerOrderList(options) {
				options.downstreamAlias = this.downstreamAlias;
				...
				getTodayCustomerOrderList(options).then(response => {
			...
			oneClickToBalance() {
				this.orderType = 4;
				this.messageText = `请您确认是否对客户“${this.downstreamAlias}”已入库的订单进行结算？`
				this.$refs.popup.open('center')
			}
		}
	}
